...
election_year = 2008 # election year
...
for(i in 1:length(district_codes)) {
  ...
  district_data_temp <- district_data_temp %>%
    ...
    mutate_at(5:(n_fields-3), as.numeric) %>%
...
district_key <- readr::read_csv(paste0("Data/electoral_districts_key_", ridings_year, ".csv"), locale = locale(encoding = "UTF-8"))
